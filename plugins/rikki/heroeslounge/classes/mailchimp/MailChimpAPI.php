...
class MailChimpAPI
{
	...
	*/
	...
	public static function subscribeNewUser($user)
	{
	    ...
	    $data = array('email_address'=>$user->email,'status'=>'subscribed', 'merge_fields' => array('FNAME' => $user->sloth->title), 'interests' => array('4a5a3da9d8' => ($user->sloth->isCaptain()), '45123ca490' => ($user->sloth->teams->count() > 0)));
	...
	{
	    $request_url = "https://us17.api.mailchimp.com/3.0/lists/8877904eb6/members";
	    $data = array('email_address'=>$user->email,'status'=>'unsubscribed', 'merge_fields' => array('FNAME' => $user->sloth->title), 'interests' => array('4a5a3da9d8' => ($user->sloth->isCaptain()), '45123ca490' => ($user->sloth->teams->count() > 0)));
	    $data_json = json_encode($data);
	...
	public static function unsubscribeExistingUser($user)
	{
		...
		$data = array('status'=>'unsubscribed', 'merge_fields' => array('FNAME' => $user->sloth->title), 'interests' => array('4a5a3da9d8' => ($user->sloth->isCaptain()), '45123ca490' => ($user->sloth->teams->count() > 0)));
		$data_json = json_encode($data);
	...
	public static function subscribeExistingUser($user)
	{
		...
		$request_url = "https://us17.api.mailchimp.com/3.0/lists/8877904eb6/members/".$email_hash;
		$data = array('status'=>'subscribed', 'merge_fields' => array('FNAME' => $user->sloth->title), 'interests' => array('4a5a3da9d8' => ($user->sloth->isCaptain()), '45123ca490' => ($user->sloth->teams->count() > 0)));
		$data_json = json_encode($data);
	...
	public static function patchExistingUser($user)
	{
		...
		$request_url = "https://us17.api.mailchimp.com/3.0/lists/8877904eb6/members/".$email_hash;
		$data = array('interests' => array('4a5a3da9d8' => ($user->sloth->isCaptain()), '45123ca490' => ($user->sloth->teams->count() > 0)));
		$data_json = json_encode($data);
